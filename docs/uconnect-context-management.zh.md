...
- 资产对象
  - 被系统模块读写的数据，例如 `Module Fact`、`Engineering Fact`、`Architecture Fact`、`Validation Fact`、`Feature Asset`、`Requirement Asset`、`Specification Asset`、`BDD Scenario Asset`、`Flow Asset`、`Skill Asset`、`Validator Asset`、registry 和 `module_asset_graph`
- 治理机制
...
- `Context Router / Loader`
  - 根据 query、diff、目标模块、Asset Graph、policy、当前消费快照和预算选择上下文资产，并输出 `ContextBundle` 精确资产清单
- `Skill Execution`
...
flowchart LR
    ...
    Registry --> FeatureItem[Feature Asset Item<br/>默认 item-only]
...
`Registry Item` 和 `module_asset_graph` 负责在不打开资产正文的情况下完成候选召回；Router 再结合 policy、当前消费快照和 budget 完成过滤、裁剪和清单生成。`ContextBundle` 只承载精确资产清单，不承载完整资产正文。`Asset Body` 是代码助手或 Skill 按清单授权从 consumption 受控读取的资产正文；`Fact` 只指事实类资产中的事实内容，只有 `Module Fact / Engineering Fact` 属于 L0。
...
- `Feature Asset`
  - 功能资产，用于描述 domain/module 下可交付的功能或能力单元；它负责功能归属、默认模块入口和跨模块参与，不重复保存需求和规格边
- `Requirement Asset`
...
- `module_asset_graph`
  - 以模块为主要入口的跨层资产图，把 `Module Fact`、`Engineering Fact`、`Architecture Fact`、`Validation Fact`、`Feature Asset`、`Specification Asset`、`Flow Asset`、`Skill Asset` 和 `Validator Asset` 连接起来；`Requirement Asset` 通过 `Specification Asset.requirement_refs` 间接进入候选集合；`BDD Scenario Asset` 通过 registry item 进入候选集合，并可通过 `bdd_refs` 从规格、验证事实或 Validator 展开；它表达“可能相关”，不表达“本次必须加载”
...
可路由正文资产默认维护两件套：`Asset Body` 与 `Asset Registry Item`。`Feature Asset` 例外，默认以 `Asset Registry Item` 表达功能归属和模块入口；只有当功能边界、能力拆解或稳定说明需要独立正文时，才补充 `Feature Asset Body`。系统不维护独立的动态指标文件。Router 在运行时把 `Asset Registry Item` 和当前加载原因融合成 `Runtime Asset Descriptor`，用于最终排序、过滤和裁剪。同优先级排序使用确定性 tie-breaker（显式边优先于推断边，再按 `confidence` 降序，最后按 `asset_id` 字典序），不依赖任何运行期累积指标，以保证同一快照下输出可复现。
...
`Asset Body` 和 `Asset Registry Item` 可以物理 colocate，但语义上仍然分离。`Feature Asset` 的 item-only 形态只保存功能归属、模块入口、适用条件和轻量关联，不承载需求、规格、BDD 或验证正文。系统不维护独立的动态指标文件；资产是否可见只由当前 build snapshot 决定。Router 不扫描 authoring 目录；只有 build 到 `consumption/context_service` 的产物才允许被 Router 和 Skill 消费。
...
- `authoring/domains/<domain>/modules/<module>/module_facts/`：存放当前模块的自动生成 L0 模块事实。
...
- `authoring/domains/<domain>/modules/<module>/feature_assets/`：存放当前模块默认负责或参与的功能归属资产。
...
- `authoring/asset_graph/`：存放以模块为入口的资产关系图和 graph validator 诊断结果。
- `authoring/asset_graph/module_asset_graph.yaml`：存放 authoring 阶段可 review 的模块资产图交换格式。
...
- `authoring/build/diagnostics/`：存放 schema、词汇、引用、graph 和 stale 检查诊断。
...
- `consumption/context_service/domains/<domain>/modules/<module>/`：存放按模块发布的可消费资产分片，包括 item-only 的 Feature registry item 和其它正文资产两件套。
- `consumption/context_service/asset_graph/`：存放发布后的当前稳定 Asset Graph。
- `consumption/context_service/asset_graph/module_asset_graph.yaml`：存放发布后的模块资产图快照。
...
- `consumption/context_service/indexes/by_module/`：存放按 module 切分的资产索引。
- `consumption/context_service/indexes/by_type/`：存放按 asset type 切分的资产索引。
- `consumption/context_service/indexes/module_asset_graph.diagnostics.yaml`：存放发布图的 graph validation 和 build gate 诊断。
...
- `consumption/context_service/adapters/`：存放面向具体实现或运行环境的消费适配层。
...
- `module_asset_graph`
  - 模块到 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等资产的引用图，也可以表达跨模块、跨流程、跨验证能力的候选关系；`BDD Scenario Asset` 作为 registry item 被发现，并通过 `bdd_refs` 关系索引参与导航
...
默认语义层级是 `domain -> module -> feature`：一个 domain 下可以有多个 module，一个 module 下可以有多个 feature。`Feature Asset` 通过 `ownership.primary_module_ref` 表达默认归属模块，通过 `ownership.module_refs` 表达跨模块参与；因此账号同步、初始化、支付这类跨模块功能不需要被拆成多个互相割裂的 issue 或 module 绑定。GitHub Project / Issue 不直接挂到 module，而是沉淀为 `Requirement Asset`；spec / plan / 设计文档沉淀为 `Specification Asset`；BDD 验收场景沉淀为 `BDD Scenario Asset`。Requirement 通过 `Specification Asset.requirement_refs` 和 coverage 字段进入上下文，Specification 通过 `Validation Fact.specification_refs` 和 `refs.specifications` 进入上下文，BDD Scenario Asset 通过自己的 registry item 进入候选集合，并可以通过 `Specification Asset / Validation Fact / Validator Asset` 的 `bdd_refs` 展开；这些引用不在 `Feature Asset` 中重复保存。PR 作为 feature、requirement 或 specification 的来源引用进入，不作为模块图边。`Skill Asset` 以 module 级为默认归属，表达某个 module 内接入、集成、上报、回归预检等任务方法；不要下沉到 feature 级。
...
- `vocabularies`
  - 统一词汇表，约束 `module_tags`、`capabilities`、`dependency_edge_type` 等命名，并包含 `edge_ontology.yaml` 定义合法的引用边词汇语义、分组与来源取值
- `schemas`
...
- SDK 接入基线版本、宿主工程基线和兼容矩阵
  ...
  - 全局只有一份工程事实来源；模块相关性通过 `package_refs / source_paths / platforms` 和 `refs.l0` 表达，不在模块目录复制
- 项目共享架构图、流程图、依赖图
...
不新增 `refs.shared`。`domains/shared/` 只是项目内共享资产的物理 domain；消费模型上仍是普通资产，通过 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.specifications / refs.flows / refs.skills / refs.validators` 等普通图边，或通过 registry item 和资产正文中的稳定引用参与消费。

### 6.1 Asset Registry

`Asset Registry Item` 是每个资产目录下的 `asset.registry.yaml`。它不是集中手写的大注册表。它为所有可被 Router 独立发现、过滤、排序、裁剪和加载的资产提供静态身份证，包括 `Module Fact`、`Engineering Fact`、`Architecture Fact`、`Validation Fact`、`Feature Asset`、`Requirement Asset`、`Specification Asset`、`BDD Scenario Asset`、`Flow Asset`、`Skill Asset` 和 `Validator Asset`。
...
- `FeatureAssetItem`
  - 这个功能属于哪个 domain、默认归属哪个 module、跨哪些 module，并关联哪些架构、验证和流程事实
...
type ArchitectureFactItem = BaseAssetItem & {
  ...
  // 该事实适用的风险点摘要
  risk_summary?: string;
  // 复核状态摘要，来自 Architecture Fact Body 的 code_binding
...
type ValidationFactItem = BaseAssetItem & {
  ...
  // 复核状态摘要，来自 Validation Fact Body 的 code_binding
  review_status?: "current" | "stale";
  // stale 时的结构或验证触点漂移原因摘要
  stale_reasons?: string[];
};
...
type FeatureAssetItem = BaseAssetItem & {
  ...
  // Feature 可读名称
  feature_name: string;
  // 默认归属模块；用于 module -> feature 的第一跳导航
  primary_module_ref: string;
  // 跨模块参与列表；一个 feature 可以覆盖多个 module
  module_refs?: string[];
  // 涉及能力
  capability_tags?: string[];
...
type SpecificationAssetItem = BaseAssetItem & {
  ...
  // 规格来源，例如 Guru spec、Guru plan、设计文档或人工维护规格
  source_provider: "guru_spec" | "guru_plan" | "design_doc" | "manual" | "other";
...
type BddScenarioAssetItem = BaseAssetItem & {
  ...
  // 关联 Feature Asset，用于回溯功能归属和模块入口
  feature_refs?: string[];
...
- `asset_registry.index.yaml`
  - 全局轻量索引，只保存 `asset_id`、`asset_type`、`domain`、`module_tags`、`intent_tags`、可选 `body_path` 和 `estimated_tokens`
...
- `by_module/<module_id>.yaml`
  - 可选分片，按模块组织与该模块相关的 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / BDD Scenario Asset / Flow Asset / Skill Asset / Validator Asset` 候选资产
...
`routing_policies` 是共享策略。它定义不同 query intent 如何加载 `Module Fact / Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Requirement Asset / Specification Asset / BDD Scenario Asset / Flow Asset / Skill Asset / Validator Asset` 等资产，以及每类请求的扩展深度、槽位要求和预算边界。
...
`Requirement Asset / Specification Asset / BDD Scenario Asset / Flow Asset / Skill Asset / Validator Asset` 也可以有 `Asset Body`，但它们不是 `Fact`，不属于 `L0 / L1 / L2` 事实分层；它们通过 registry、`module_asset_graph` 和稳定引用参与路由与导航。
...
代码结构漂移检查只作用于人工维护、且会被代码结构影响的事实资产：

- `Architecture Fact`
  - 检查架构约束绑定的模块、路径、入口、依赖或包边界是否发生结构漂移
- `Validation Fact`
  - 检查验证口径依赖的模块、路径、验证触点、日志/callback/event/API 等证据通道是否发生结构漂移

`Module Fact` 和 `Engineering Fact` 每次 build 都从当前工程自动扫描生成，代表当前状态，不做 stale 判断。`Flow Asset` 不直接保存代码路径或入口符号，只引用 `Architecture Fact / Validation Fact` 并继承它们的复核状态。`Validator Asset` 不直接保存代码结构绑定，只引用 `Validation Fact`；它自身只通过实际验证执行或 validator health check 发现工具、设备、selector、步骤或证据解析漂移。
...
type FactCodeBinding = {
  ...
  // 该事实绑定的源码、配置或资源路径，用于触发结构复核
  source_paths: string[];
  // L0 scanner 在确认时生成的结构签名
...
`Architecture Fact` 通过 `code_binding.source_commit / source_paths / structural_signature` 绑定到具体代码结构。build 时若发现当前 L0 scanner 产出的结构签名与 `code_binding.structural_signature` 不一致，则把 `code_binding.review_status` 标记为 `stale` 并写入 `stale_reasons`。这里的结构性变更指路径存在性、文件移动、关键符号入口、包/asmdef 边界或声明依赖等可由扫描器稳定识别的结构差异，不包括“设计语义是否仍然正确”这类只能人工判断的问题。

`stale` 是复核信号，不是自动失效状态。默认策略是：Router 仍可返回 `stale` 的 `Architecture Fact`，但必须降低其排序权重，在 `ContextBundle` 或 diagnostics 中标明 `review_status: stale` 与触发原因；当某个 routing policy 或 Skill 要求 `architecture_fact` 必须是强约束事实时，policy 可以声明 `allow_stale_required: false`，此时 `stale` 资产只能作为参考上下文，不能单独满足 required slot。人工复核通过后刷新 `code_binding.source_commit`、`structural_signature` 与 `review_status`。

这是 `L1` 的轻量防漂移机制：`L1` 是设计性软知识，无法像编译一样自动判定是否仍然成立，因此只标记待复核，由人确认后刷新 `code_binding`。
...
`Validation Fact` 的 `code_binding` 只绑定验证口径依赖的代码结构和验证触点，例如模块入口、配置触点、日志 tag、callback 名称、event 名称、API/SQL 结果 schema 或设备状态观测点。build 时将当前 L0 scanner 与验证触点扫描结果生成新的结构签名，并与 `code_binding.structural_signature` 对比；不一致时标记 `review_status: stale` 并写入 `stale_reasons`。

`Validator Asset` 只通过 `validates` 引用 `Validation Fact`，不重复保存这些代码结构绑定。Router 返回 Validator Asset 时应同时暴露其关联 `Validation Fact` 的 `review_status`；如果关联的 `Validation Fact` 为 `stale`，Validator Asset 只能作为候选验证流程，不能证明当前验证口径仍然有效。

### 7.5 BDD Scenario Asset
...
### 7.6 Feature / Requirement / Specification Assets

`Feature Asset`、`Requirement Asset` 和 `Specification Asset` 共同描述“需求、规格、实现和验证”的闭环。

职责链路固定如下：

- `Requirement Asset`：需要做什么
- `Specification Asset`：设计承诺怎么做，并映射 `Requirement Asset`
- `BDD Scenario Asset`：真实场景分析是什么，独立保存 Given / When / Then 验收语义
...
- `Feature Asset`：功能归属和模块入口，不重复保存需求和规格边

- `Requirement Asset`
  - 回答“需要做什么”
  - 来源是 GitHub Project / Issue、用户意图、关键讨论结论和验收标准
  - 不直接绑定 module，不作为 `module_asset_graph.refs.*` 的第一跳边，由 `Specification Asset.requirement_refs` 引入
- `Specification Asset`
  - 回答“设计上承诺怎么做”
  - 来源是 spec / plan / 设计文档
  - 通过 `requirement_refs / bdd_refs / requirement_coverage / implementation_coverage / validation_coverage` 映射需求、BDD 场景资产、实现证据和验证事实
- `Feature Asset`
  - 回答“最终归属到哪个可交付功能”
  - 绑定默认 module 和参与 module，关联 `Architecture Fact / Validation Fact / Flow Asset`
- `Implementation / Validation evidence`
  - 回答“真正做了什么、怎么证明”
  - 以 PR、commit、执行证据、BDD Scenario Asset 结果、validator evidence bundle 等引用形式进入
...
type FeatureAssetBody = {
  ...
  // 功能归属和模块入口
  ownership: {
    // 默认归属模块，用于 module -> feature 第一跳
    primary_module_ref: string;
    // 参与模块列表，用于跨模块 feature
    module_refs?: string[];
  };
  // 功能能力标签
  capability_tags?: string[];
  // Feature 只关联事实和流程，不重复保存 Requirement / Specification 直接边
  linked_assets?: {
    // 关联 Architecture Fact
    architecture_fact_refs?: string[];
    // 关联 Validation Fact
...
type SpecificationAssetBody = {
  ...
  source: {
    // 来源提供方
    provider: "guru_spec" | "guru_plan" | "design_doc" | "manual" | "other";
...
`Specification Asset` 是闭环核心：它不替代 `Architecture Fact` 或 `Validation Fact`，而是把需求意图、设计承诺、架构约束、验证口径、BDD Scenario Asset 和实现证据连接起来。UConnect GuruJarvis Adapter 应把 Guru Jarvis 的 `proposal.md`、`spec.md`、`plan.md`、`implementation.md`、BDD Scenario 矩阵和 evidence refs 转换为这些结构化资产，而不是让 Router 直接解析 Markdown / Obsidian。
...
`module_asset_graph` 是以模块为主要入口的资产图，不局限于 L0。`L0` 提供自动生成的模块事实、依赖事实和工程事实，是资产图的主要输入之一；资产图负责把模块入口连接到 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等跨层资产，让 Router 能从一个 query、diff 或目标模块稳定走到可用候选资产。`BDD Scenario Asset` 不需要专门的 BDD 模块图边；它作为普通 registry item 被发现，并通过 `bdd_refs` 与规格、验证事实和 Validator 建立关系。
...
flowchart TB
    ...
    FeatureRefs[Feature refs] --> AssetGraph
    SpecRefs[Specification refs] --> AssetGraph
...
- `refs.l0`
  - 当前模块对应的 L0 代码与工程事实，例如相关 `Module Fact`、SDK 版本基线、插件版本、平台构建配置和兼容矩阵
- `refs.l2`
...
- `refs.features`
  - 当前模块相关的 Feature Asset，例如该模块默认负责或参与的可交付功能；Feature 只承担功能归属和模块入口，不重复保存需求和规格直接引用
- `refs.specifications`
  - 当前模块相关的 Specification Asset，例如 spec / plan / 设计文档沉淀的设计承诺；Requirement 的“需要做什么”通过 Specification 的 coverage 字段映射到“设计上承诺怎么做、真正做了什么、怎么证明”
这组边的目标，是让 agent 能从模块入口稳定走到 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等可用候选资产，而不是退回到模糊匹配。`BDD Scenario Asset` 由 registry index 直接提供候选，并通过 `bdd_refs` 关系索引参与进一步导航，不需要额外的模块图专门边。`module_asset_graph` 不负责最终上下文选择；它提供候选集合，`routing_policies`、build 后的 registry index 和 `Context Fuser` 再按 intent、平台、场景和预算修剪候选集合，以获得最大上下文信噪比。

边的类型不是隐式约定，而是受治理的词汇语义。它作为 shared asset 维护在 `vocabularies/edge_ontology.yaml` 中，定义合法的边分组、语义和来源取值。这里沿用 `edge_ontology.yaml` 文件名，但语义上它是引用边词汇表，不是复杂知识本体。新增边类型属于词汇扩展，必须先更新该词表并通过评审，再被资产引用；这样系统的可扩展性（把更多高价值关系挂成边）建立在受控词表之上，而不是散落的未治理约定之上。
...
build gate 必须按 `edge_ontology.yaml` 校验所有 `refs` 下的分组键、允许的目标资产类型集合和 `source` 取值；未在词表中声明的边分组或来源值一律阻断发布。Router 和文档可以使用 `refs.l0` 这类概念路径表达扩展方向，但实现读取时必须解析为 `refs["l0"]`。这是边可扩展性的治理边界：`Feature Asset` 是功能归属和模块入口，不重复保存需求和规格边；GitHub Project / Issue 沉淀为 `Requirement Asset`，spec / plan / 设计文档沉淀为 `Specification Asset`，再通过 `module_asset_graph.refs.specifications` 进入第一跳候选，并通过 `Specification Asset.requirement_refs` 展开 Requirement。BDD 验收场景沉淀为 `BDD Scenario Asset`，作为普通 registry item 进入候选集合，并可通过 `Specification Asset / Validation Fact / Validator Asset` 的 `bdd_refs` 展开。PR 作为 `Feature Asset.source_refs`、`Requirement Asset.source_refs` 或 `Specification Asset.source_refs` 的来源引用保留，不进入默认图边。其它 tools 引入和业务事件类边（如复核、检查节点）都应通过同一套词汇扩展流程接入，而不是绕过词表新增隐式边。

`module_asset_graph.nodes[].module_fact_ref` 是当前模块自身的主 `Module Fact` 引用；`refs.l0` 可以额外引用与当前模块相关的其它 L0 事实，包括相关 `Module Fact` 和 `Engineering Fact`。Router 定位目标模块时优先使用 `module_fact_ref`，做扩展和上下文补充时再使用 `refs.l0`。
...
`Module Fact` 只保存自动生成的包级事实。`Engineering Fact` 保存可从工程配置恢复的版本、平台和兼容矩阵事实。`module_asset_graph` 保存人工维护或半自动维护的候选导航边，从而让 `L0` 保持轻量，同时让 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等资产的绑定关系独立更新。`BDD Scenario Asset` 的发现入口是 registry item，关系导航由 `bdd_refs` 提供。
...
- 把 `module:<package_name>` 映射到对应的 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等资产
...
`module_asset_graph` 逻辑上是一张小型图数据库，不是单纯的大 YAML 文件。authoring 阶段可以用 `module_asset_graph.yaml` 作为可 review 的交换格式；build 发布到 consumption 后必须生成可查询索引，例如 `nodes`、`edges`、`by_module`、`by_asset`、`by_ref_group` 和 diagnostics 分片。Router 面向发布后的索引读取，不直接扫描 authoring 工作文件。

规范数据模型由三部分组成：

- `nodes`
  - 模块节点表，保存 `module_id`、`module_fact_ref`、包路径、依赖关系和按边分组生成的 `refs` 投影视图
- `edges`
  - 规范化引用边表，保存 `from_node_id`、`ref_group`、`to_asset_id`、`source`、`reason`、`confidence` 和 diagnostics
- `indexes`
  - build 生成的反向索引，例如 `asset_id -> modules`、`module_id -> refs`、`ref_group -> edges`，用于 Router 快速定位候选
...
nodes:
  ...
  - module_id: module:<package_name>
    ...
    refs:
      ...
      # Feature Asset 引用
      features:
        - asset_id: feature:<domain>/<feature_name>
          source: registry
          reason:
          confidence:
      # Specification Asset 引用
      specifications:
        - asset_id: spec:<domain>/<spec_name>
          source: registry
          reason:
          confidence:
      # Flow Asset 引用
    ...
    metadata:
      # 最近更新时间
      updated_at:
# 规范化引用边表；refs 是它的按模块分组投影视图
edges:
  - edge_id:
    from_node_id: module:<package_name>
    ref_group: specifications
    to_asset_id: spec:<domain>/<spec_name>
    source: registry
    reason:
    confidence:
    diagnostics: []
...
`refs` 使用分组结构，而不是把所有资产放进一个数组。Router 按 query intent 直接选择资产类型，避免先加载无关资产再过滤。`refs` 是面向读取的投影视图，`edges` 是规范化边表；build gate 必须校验二者一致。

`refs.*[]` 只保存导航所需的稳定资产 ID、引用来源、原因和置信度。Router 不在图里做最终排序，而是在运行时把 registry item 与加载原因融合为 `Runtime Asset Descriptor`，并用确定性 tie-breaker 排序。图数据库化的目的不是让 Router 执行复杂推理，而是保证深层检索时每一跳都有可校验边、有反向索引、有确定性输入快照。
...
- `asset.registry.yaml`
  - 各资产目录中的静态身份证，包含 `asset_id`、`asset_type`、摘要、路径、适用意图、平台、场景、模块、能力标签、验证触点、Fact 复核状态和预算信息
- `vocabularies`
  - `module_tags`
  - `capabilities`
...
type ModuleAssetGraphBuildInput = {
  ...
  // Feature Asset 输入目录
  featureAssetsDir: string;
  // Requirement Asset 输入目录
  requirementAssetsDir: string;
  // Specification Asset 输入目录
  specificationAssetsDir: string;
  // BDD Scenario Asset 输入目录
  bddScenariosDir: string;
  // Flow Asset 输入目录
...
3. 读取当前 L0 scanner 产出的代码结构签名和验证触点签名
4. 对 `Architecture Fact / Validation Fact` 执行 code drift check，刷新 registry item 中的 `review_status / stale_reasons` 摘要
5. 读取 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / BDD Scenario Asset / Flow Asset` 的 registry 类型字段或关联字段
6. 使用 `module_tags`、`primary_module_ref`、`module_refs`、`feature_refs`、`requirement_refs`、`bdd_refs`、`involved_modules`、`architecture_fact_refs`、`validation_fact_refs` 和 `validator_bindings` 将上层资产挂到候选模块或关系索引
7. 读取各资产目录的 `asset.registry.yaml`，根据资产声明的 `asset_type`、`applies_to`、`module_tags`、`capabilities`、`validation_touchpoints` 生成 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.specifications / refs.flows / refs.skills / refs.validators`；`BDD Scenario Asset` 不生成专门 `refs.*` 分组，而是进入 registry index 和 `bdd_refs` 关系索引
8. 没有显式适用条件、无法通过 registry 类型字段或规则绑定到模块的资产，不生成模块引用边
9. 对每条引用边计算 `source / reason / confidence`
10. 使用 `confidence` 标记引用边可信度，不在候选图阶段提前裁剪可校验候选
11. 执行 graph validation，检查悬空引用、重复引用、非法 asset id、缺失 registry 类型字段和不可消费路径
12. 输出 `module_asset_graph.yaml`、规范化 `edges`、发布索引和 diagnostics

code drift check 只检查 `Architecture Fact / Validation Fact`。`Flow Asset` 通过 `architecture_fact_refs / validation_fact_refs` 继承相关 Fact 的 `review_status`；`Validator Asset` 通过 `validates` 继承相关 `Validation Fact` 的 `review_status`。build 不对 Flow / Validator 做代码结构签名比较。
...
- `source: registry`
  - 来自 `asset.registry.yaml` 显式声明，或 registry 类型字段推断；`Engineering Fact` 可由 `source_paths` 推断，`Architecture Fact / Validation Fact` 由 `module_tags / validation_touchpoints / review_status` 摘要推断，`Feature Asset` 由 `primary_module_ref / module_refs / linked_assets` 推断，`Specification Asset` 由 `feature_refs / requirement_refs / architecture_fact_refs / validation_fact_refs / bdd_refs` 推断，`BDD Scenario Asset` 由 `applies_to / scope / linked_assets` 进入 registry index，不生成专门模块图边，`Flow Asset` 由 `involved_modules / architecture_fact_refs / validation_fact_refs` 推断，`Validator Asset` 由 `validates / bdd_refs` 推断
- `source: manual`
  - 来自人工确认的显式绑定
- `source: policy`
...
1. 固定输入快照
  ...
  - 读取 `Module Fact`
  - 读取 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / BDD Scenario Asset / Flow Asset` registry 类型字段
  - 读取各资产目录的 `asset.registry.yaml`
  - 读取 `routing_policies / vocabularies`
2. 构建候选图
  - 生成 `module_id -> Module Fact` 索引
  - 生成模块依赖边
  - 生成 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.specifications / refs.flows / refs.skills / refs.validators`，并生成 BDD registry / `bdd_refs` 关系索引
...
6. 发布到消费服务
  - 将通过门禁的 registry item、非 item-only 资产正文、`module_asset_graph`、registry index、Skill 摘要、validator 摘要和相关索引发布到 `consumption/context_service`
...
- `asset_graph/module_asset_graph.yaml`
  ...
  - Router 用它读取 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.specifications / refs.flows / refs.skills / refs.validators`
...
- `context:asset-graph:build`
  - 通过 build gate 后同步 registry item、非 item-only 资产正文、`module_asset_graph`、diagnostics 和 graph snapshot 到 `consumption/context_service`
...
- 从当前 build snapshot 中加载 `Module Fact / Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / BDD Scenario Asset / Flow Asset / Skill Asset / Validator Asset` 的 registry item、摘要和必要片段
...
type RoutingPolicy = {
  ...
  expansionRules: Array<{
    ...
    // 扩展使用的 refs 边
    via: "refs.l0" | "refs.l1" | "refs.l2" | "refs.features" | "refs.specifications" | "refs.flows" | "refs.validators" | "refs.skills";
    // 最大扩展深度
    maxDepth: number;
  }>;
...
`expansionRules` 沿 `module_asset_graph.refs.*` 做第一跳和图边扩展。`bodyRefExpansionRules` 用于 build 阶段从资产正文抽取并发布到 registry / graph index 的稳定引用，例如 `Specification Asset.requirement_refs`、`Specification Asset.bdd_refs`、`Validation Fact.bdd_refs` 和 `Validator Asset.bdd_refs`。Router 运行时不为了扩展规则自由打开资产正文；它只消费当前快照内已索引的引用关系。`Requirement Asset` 不作为 `module_asset_graph.refs.*` 第一跳边，只能通过 `Specification Asset.requirement_refs` 或显式请求进入；`BDD Scenario Asset` 不需要专门的 `module_asset_graph.refs.*` 边，它通过 registry item 进入候选集合，也可以通过 `bdd_refs` 从规格、验证事实或 Validator 继续展开。
...
6. 按 policy 扩展资产
  ...
  - 根据 `maxDepth` 控制遍历深度
  - `Requirement Asset` 只能由 `Specification Asset.requirement_refs` 或显式请求引入，不作为 `module_asset_graph.refs.*` 的第一跳边
  - `BDD Scenario Asset` 可以由 registry item、已索引的 `bdd_refs` 或显式请求引入
7. 执行候选过滤
...
6. 输出 `ContextBundle`
  - 资产顺序固定为 `module_fact -> engineering_fact -> architecture_fact -> validation_fact -> feature_asset -> specification_asset -> bdd_scenario_asset -> requirement_asset -> flow_asset -> skill_asset -> validator_asset`
  - 同类型内按 priority 降序排列
...
`Validator Asset` 不直接保存代码路径、入口符号、日志 selector、callback 名称或 event/schema 这类代码结构绑定；这些绑定属于 `Validation Fact.code_binding`。Validator 只描述“如何执行当前验证流程”：使用哪些工具、设备要求、执行步骤、证据提取器和结果解释规则。它的代码有效性来自所引用 `Validation Fact` 的 `review_status`；它自身的漂移通过验证执行、validator health check 或证据提取失败发现。
...
规模变大后，不允许一次性把所有资产塞给相关 Skill。上下文应该按照当前需求采用渐进式选择，让 Router 先用低成本资产缩小范围，再为高价值资产生成精确清单和正文读取建议。

渐进式遍历解决的是现实中的时效性问题：

- 大型项目中资产数量多，全量加载会直接消耗上下文窗口
- Architecture Fact / Validation Fact 质量高但体积大，必须按需读取正文
...
flowchart TB
    ...
    PolicyFilter --> NeedDetails{"Need Detail Pointers?<br/>是否需要正文读取建议？"}
    NeedDetails -->|L1| L1["Select Architecture Facts<br/>选择架构事实清单项"]
    NeedDetails -->|L2 / BDD / Spec / Flow| L2["Select Validation / BDD / Specification / Flow Assets<br/>选择验证 BDD 规格与流程资产清单项"]
    NeedDetails -->|Skill / Validator| Shared["Select Skill / Validator Assets<br/>选择 Skill 与验证资产清单项"]
    NeedDetails -->|no| Budget["Budget Check<br/>预算检查"]
    L1 --> Budget
    L2 --> Budget
    Shared --> Budget
...
3. 按 query intent 决定第一批资产类型
  - `smoke_test` 先取 `Module Fact + refs.l2 + refs.validators`，再按 registry item 和 `bdd_refs` 筛选相关 BDD Scenario Asset
  - `impact_analysis` 先取 `Module Fact + refs.features + refs.specifications + refs.l1`，再按 registry item 和 `bdd_refs` 筛选相关 BDD Scenario Asset
  - `integration_validation` 先取 `refs.features + refs.specifications + refs.l1 + refs.l2 + refs.flows + refs.validators`，再按 registry item 和 `bdd_refs` 筛选相关 BDD Scenario Asset
...
6. 生成资产清单项和正文读取建议
  - 只为通过筛选且满足 policy 的资产生成摘要、读取指针、hash 和 `recommendedFetchMode`
  - `refs.features` 对应的是 Feature Asset，用于功能归属和模块入口，不负责引入 Requirement / Specification
  - `refs.specifications` 对应的是 Specification Asset，Requirement 的覆盖状态和实现证据由 Specification 的 coverage 字段引入
  - BDD Scenario Asset 不对应专门 `refs.*` 分组；Router 通过 registry item、`applies_to`、场景标签和 `bdd_refs` 关系索引选择其摘要和读取指针；代码助手需要完整场景分析和 Given / When / Then 时再按清单授权读取正文
...
可路由正文资产固定为两件套：`Asset Body` 和 `Asset Registry Item`。`Feature Asset` 默认 item-only，用 registry item 表达功能归属、模块入口和轻量关联；当 Feature 需要稳定正文时，可以补充 `Feature Asset Body`，但这不是默认要求。系统不维护独立的动态指标文件，避免字段归属出现二义性，也避免引入“更新需重新 build 才生效”的伪动态机制。
...
`Feature Asset` item-only 时必须通过 registry item 保存 `primary_module_ref / module_refs / capability_tags / architecture_fact_refs / validation_fact_refs / flow_asset_refs / source_refs` 等轻量字段，不能把需求、规格、BDD 或验证正文塞进 item。需要这些正文时，应分别提升为 `Requirement Asset / Specification Asset / BDD Scenario Asset / Validation Fact`。
...
- `Architecture Fact / Validation Fact` 的 `code_binding` 是否能与当前 L0 结构签名和验证触点签名比较，若结构签名不一致则把 `review_status` 标记为 `stale` 并写入 `stale_reasons`
- `Feature Asset` 是否通过 `primary_module_ref / module_refs` 绑定模块，并通过 `architecture_fact_refs / validation_fact_refs / flow_asset_refs` 关联事实；Feature 可以跨模块，但必须有一个默认归属模块，不保存 Requirement / Specification 直接引用
...
- `module_asset_graph.refs.*` 的边分组、目标资产类型集合和 `source` 取值是否符合 `vocabularies/edge_ontology.yaml`
- registry index 是否能解析到 schema、summary 和 body path
- required refs 是否存在
- shared assets 中被消费的资产是否已注册为普通资产，并通过 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.specifications / refs.flows` 等普通图边、registry item 或稳定正文引用被发现
...
4. 对受影响模块相关的 `Architecture Fact / Validation Fact` 重跑 code drift check
5. 重新读取受影响模块相关的 `asset.registry.yaml`
6. 重建受影响模块在 `module_asset_graph` 中的节点和 refs
7. 重新生成受影响的 `by_domain / by_module / by_type` registry index 分片
...
GraphRAG 用于增强 `Engineering Fact / Architecture Fact / Validation Fact / BDD Scenario Asset` 的跨资产关联检索，例如模块、能力、工程事实、架构事实、验证事实、BDD 场景资产、版本基线、流程资产和验证工具绑定之间的关系检索。它不应替代当前的 `L0` 自动生成和 `Context Consumption Service` 主路径。
...
2. 自动生成轻量 `L0`，补充 `Module Fact -> Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Specification Asset / Flow Asset / Skill Asset / Validator Asset` 等资产的导航边，并把 `BDD Scenario Asset` build 进 registry index 与 `bdd_refs` 关系索引
