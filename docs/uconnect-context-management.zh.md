...
5. 读取 `Engineering Fact / Architecture Fact / Validation Fact / Flow Asset` 的 registry 类型字段
6. 使用 `module_tags`、`involved_modules`、`architecture_fact_refs`、`validation_fact_refs` 和 `validator_bindings` 将上层资产挂到候选模块
7. 读取各资产目录的 `asset.registry.yaml`，根据资产声明的 `asset_type`、`applies_to`、`module_tags`、`capabilities`、`validation_touchpoints` 生成 `refs.l0 / refs.l1 / refs.l2 / refs.flows / refs.skills / refs.validators`
...
- `source: registry`
  - 来自 `asset.registry.yaml` 显式声明，或 registry 类型字段推断；`Engineering Fact` 可由 `source_paths` 推断，`Architecture Fact / Validation Fact` 由 `module_tags / validation_touchpoints / review_status` 摘要推断，`Flow Asset` 由 `involved_modules / architecture_fact_refs / validation_fact_refs` 推断，`Validator Asset` 由 `validates` 推断
- `source: manual`
  - 来自人工确认的显式绑定
- `source: policy`
