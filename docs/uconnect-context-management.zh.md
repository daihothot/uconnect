...
- `asset_graph`
  - 当前消费快照内的资产关系图，默认以 `Behavior Asset` 为导航中心，把行为连接到 `Requirement / Specification / BDD / Validation / Validator / Architecture / Module / Engineering / Flow / Skill / Evidence` 等资产；`Requirement / Specification / BDD` 等高价值资产也可以作为显式入口，再通过 `relates_to_behavior` 回到行为中心
...
- `asset_graph`
  - 以 registry 中的资产 ID 为节点，保存 `Behavior Asset` 到 `Requirement / Specification / BDD / Validation / Validator / Architecture / Module / Engineering / Flow / Skill / Evidence` 的关系边；`Requirement / Specification / BDD` 也可以作为显式入口，再通过 `relates_to_behavior` 回到行为中心
...
- `Feature Asset`：产品或交付归属，不重复保存需求、规格和模块导航边

- `Requirement Asset`
  - 回答“需要做什么”
  - 来源是 GitHub Project / Issue、用户意图、关键讨论结论和验收标准
  - 不直接绑定 module，默认通过 `Specification Asset.requirement_refs` 或 `asset_graph.relates_to_behavior` 进入行为中心
- `Specification Asset`
  - 回答“设计上承诺怎么做”
  - 来源是 spec / plan / 设计文档
...
- `Feature Asset`
  - 回答“最终归属到哪个可交付功能”
  - 关联 `Behavior Asset / Architecture Fact / Validation Fact / Flow Asset`，不承担导航根职责
...
- `Architecture Fact / Validation Fact` 即使质量很高，也缺少从行为到约束和验证的稳定引用边
- 每次都把所有资产塞给 Skill 会造成 token 爆炸，并降低上下文质量
- 只靠自然语言标题或模糊匹配查找资产，会让召回结果不稳定
- 流程资产、验证口径和验证工具绑定无法跟具体行为建立明确关系
- 耦合严重的工程如果从 module 出发，容易把多个无关行为一并召回，导致上下文爆炸

因此，Asset Graph 的设计原则是：**行为作为默认导航中心，模块只作为实现表面边，运行时导航走确定性图边。**

### 7.2 局部架构

```mermaid
flowchart TB
    Code[Unity / UPM Codebase] --> Scanner[Code Fact Scanner]
    Scanner --> ModuleFact[Module Fact]
    Scanner --> EngineeringFact[Engineering Fact]
    Scanner --> CodeGraph[CodeGraph / Dependency Graph]

    Adapter[Adapter / Authoring Projection] --> Behavior[Behavior Asset]
    Adapter --> UpperAssets[Requirement / Specification / BDD / Validation / Validator / Architecture / Flow / Skill]

    Registry[Asset Registry Index] --> AssetGraph[asset_graph]
    Behavior --> AssetGraph
    ModuleFact --> AssetGraph
    EngineeringFact --> AssetGraph
    CodeGraph --> AssetGraph
    UpperAssets --> AssetGraph

    AssetGraph --> GraphCheck[Graph Validator]
    GraphCheck --> PublishedGraph[Published Asset Graph]

    Request[Query / Diff / Issue / BDD / Spec] --> Router[Context Router / Loader]
    Registry --> Router
...
- 生成链路
  - 从 Unity / UPM 工程自动生成 `Module Fact / Engineering Fact` 和代码依赖图
  - 从 Adapter 投影、registry item、资产正文稳定引用和治理规则生成资产关系边
  - 通过 graph validator 检查节点存在性、边类型、路径有效性和状态字段完整性
  - 通过 build gate 生成稳定 Asset Graph 到 `Context Consumption Service`
- 使用链路
  - Router 接收 `ContextLoadRequest`
  - Router 从已发布资源中读取 registry index、`asset_graph` 和相关分片索引
  - Router 先解析行为入口，再沿受控边扩展候选资产
  - Router 组装 `ContextBundle`
  - Skill 通过 `ContextBundle` 间接消费 Asset Graph 导航结果

### 7.3 图节点与引用边

引用边统一维护在 `authoring/asset_graph/asset_graph.yaml` 中。边类型不是隐式约定，而是受治理的词汇语义，定义在 `vocabularies/edge_ontology.yaml` 中。新增边类型属于词汇扩展，必须先更新该词表并通过评审，再被资产引用。

核心边类型：

- `has_requirement`
  - 行为或规格到需求资产，回答这个行为满足哪些“需要做什么”
- `specified_by`
  - 行为或需求到规格资产，回答这个行为由哪些设计承诺定义
- `has_bdd`
  - 行为、规格、验证事实或 Validator 到 BDD 场景，回答有哪些真实场景和验收语义
- `validated_by`
  - 行为、规格或 BDD 到验证事实，回答这个行为需要验证什么
- `executed_by`
  - 行为、验证事实或 BDD 到 Validator，回答怎么执行验证
- `constrained_by`
  - 行为、规格或流程到架构事实，回答这个行为受哪些设计约束影响
- `implemented_by`
  - 行为、规格或流程到模块事实和工程事实，回答这个行为涉及哪些实现表面和工程基线
- `evidenced_by`
  - 行为、规格、验证事实或 Validator 到 evidence / artifact ref，回答有哪些实现或验证证据
- `belongs_to_feature`
...
- `relates_to_behavior`
  - Requirement、Specification、BDD、Validation、Validator、Feature 或 Module 回到 Behavior，用于非行为入口回到默认导航中心
...
### 7.4 `asset_graph` 构建设计

`asset_graph` 是索引对象，不是执行模块。它本身不理解业务，也不生成验证计划。真正执行构建的是 `asset_graph builder`，它负责把 registry 节点、Adapter 关系投影、资产正文稳定引用和自动扫描结果合并成 Router 可消费的候选导航图。

#### 7.4.1 设计目标

`asset_graph` 解决的是“从行为稳定走到上下文资产，并允许高价值入口回到行为中心”的问题。

它的目标是：

- 把 `behavior:<domain>/<behavior_name>` 映射到对应的需求、规格、BDD、验证、架构、实现表面、流程、Skill 和 Validator
- 允许 `Requirement / Specification / BDD` 作为显式入口，并通过 `relates_to_behavior` 回到行为中心
- 为 Router 提供确定性的行为级第一跳导航入口
- 记录每条引用边的来源、置信度和校验状态
- 让 `Module Fact` 保持自动生成和轻量，不把上层语义塞回代码事实
...
`asset_graph` 逻辑上是一张小型图数据库，不是单纯的大 YAML 文件。authoring 阶段可以用 `asset_graph.yaml` 作为可 review 的交换格式；build 发布到 consumption 后必须生成可查询索引，例如 `nodes`、`edges`、`by_behavior`、`by_asset`、`by_edge_type` 和 diagnostics 分片。Router 面向发布后的索引读取，不直接扫描 authoring 工作文件。

规范数据模型由三部分组成：

- `nodes`
  - 资产节点表，保存来自 registry index 的 `asset_id`、`asset_type` 和是否为入口节点
- `edges`
  - 规范化引用边表，保存 `from_asset_id`、`edge_type`、`to_asset_id`、`source`、`status`、`status_source`、`reason`、`confidence` 和 diagnostics
- `indexes`
  - build 生成的反向索引，例如 `behavior_id -> edges`、`asset_id -> edges`、`edge_type -> edges`，用于 Router 快速定位候选
...
# 规范化引用边表
edges:
  - edge_id: edge:behavior:<domain>/<behavior_name>:validated_by:validation:<domain>/<validation_name>
    ...
    reason: behavior validation imported from adapter projection
    confidence: 0.95
    diagnostics: []
...
`edges` 只保存导航所需的稳定资产 ID、边类型、引用来源、状态、原因和置信度。Router 不在图里做最终排序，而是在运行时把 registry item 与加载原因融合为 `Runtime Asset Descriptor`，并用确定性 tie-breaker 排序。图数据库化的目的不是让 Router 执行复杂推理，而是保证深层检索时每一跳都有可校验边、有反向索引、有确定性输入快照。

#### 7.4.3 输入数据源

`asset_graph builder` 的输入分为三类。除 `Module Fact / Engineering Fact` 这类自动生成事实外，上层资产的 registry item、关系边和复核状态都不再被称为人工维护输入；它们是外部生产端、Adapter、验证回流或人工 authoring 写入后的结构化投影。
...
- `AdapterAssetProjection`
  - 外部生产端或 authoring 资产经过 Adapter admission gate 后生成的 registry item 投影
  - 覆盖 `BehaviorAssetItem / ArchitectureFactItem / ValidationFactItem / FeatureAssetItem / SpecificationAssetItem / BddScenarioAssetItem / FlowAssetItem / ValidatorAssetItem / RequirementAssetItem`
  - 只包含 build 和 Router 初筛需要的轻量字段，例如 `asset_id`、`asset_type`、`applies_to`、`module_tags`、`capability_tags`、`review_status`、`status_source`、`body_path` 和 `token_estimate`
- `AdapterGraphProjection`
  - 外部生产端或 authoring 资产经过 Adapter admission gate 后生成的关系边投影
  - 覆盖 `behavior_refs`、`feature_refs`、`requirement_refs`、`specification_refs`、`bdd_refs`、`architecture_fact_refs`、`validation_fact_refs`、`flow_asset_refs`、`validator_refs`、`implementation_evidence_refs`、`module_refs` 和 `involved_modules` 等关系
  - 每条边必须包含 `from_asset_id`、`to_asset_id`、`edge_type`、`source`、`status`、`status_source`、`confidence` 和 `reason`
...
- `Behavior Asset Body`
  - `aliases / scope / linked_assets / source_refs` 等用于建立行为中心入口和关系边的稳定字段
- `vocabularies`
  - `asset_types`
  - `edge_ontology`
  - `query_intents`
...
`asset_graph builder` 接收一组输入目录和当前代码提交号，输出一份可校验的 `asset_graph.yaml`。

输入：

```ts
type AssetGraphBuildInput = {
  // 本次构建依据的代码提交号
  sourceCommit: string;
  // Behavior Asset 输入目录
  behaviorAssetsDir: string;
  // Module Fact 输入目录
  ...
  // Adapter 或生产端输出的资产 registry 投影目录
  adapterAssetProjectionsDir: string;
  // Adapter 或生产端输出的关系边投影目录
  adapterGraphProjectionsDir: string;
...
```ts
type AssetGraphBuildOutput = {
  // 输出的 asset_graph 文件路径
  graphPath: string;
  // 构建诊断信息
  diagnostics: Array<{
    // 诊断级别
...
1. 读取所有 `asset.registry.yaml`，建立 `asset_id -> Asset Registry Item` 索引
2. 读取 `Behavior Asset`，建立 `behavior_id -> Behavior Asset` 和别名索引
3. 读取自动生成的 `Module Fact / Engineering Fact`，作为实现表面和工程基线候选节点
4. 合并 Adapter 已写入的 `AdapterAssetProjection`
5. 读取 Adapter 已写入的 `AdapterGraphProjection`，获得资产之间的显式关系边
6. 从资产正文稳定引用中抽取关系投影，例如 `BehaviorAsset.linked_assets`、`SpecificationAsset.requirement_refs / bdd_refs`、`ValidationFact.bdd_refs`、`ValidatorAsset.validates / bdd_refs`
7. 校验 `review_status / stale_reasons / status_source` 摘要；UConnect build 不执行代码漂移检查
8. 根据 registry item、Behavior scope、Adapter 投影和 policy 规则补充低风险候选边，例如 `Behavior -> Module Fact`、`Behavior -> Engineering Fact`、`Behavior -> Skill Asset`
9. 没有显式适用条件、无法通过 Behavior、registry 类型字段、Adapter 投影或规则绑定的资产，不生成默认导航边
10. 对每条引用边计算或校验 `edge_type / source / status / status_source / reason / confidence`
11. 使用 `confidence` 标记引用边可信度，不在候选图阶段提前裁剪可校验候选
12. 执行 graph validation，检查悬空引用、重复引用、非法 asset id、缺失 registry 类型字段和不可消费路径
13. 输出 `asset_graph.yaml`、规范化 `edges`、发布索引和 diagnostics
...
- `source: registry`
  - 来自 `asset.registry.yaml` 显式声明，或 registry 类型字段推断；`Behavior Asset` 由 `aliases / scope / linked_assets` 推断，`Engineering Fact` 可由 `source_paths` 推断，`Architecture Fact / Validation Fact` 由 `module_tags / validation_touchpoints / review_status` 摘要推断，`Feature Asset` 由 `behavior_refs / linked_assets` 推断，`Specification Asset` 由 `feature_refs / requirement_refs / architecture_fact_refs / validation_fact_refs / bdd_refs` 推断，`BDD Scenario Asset` 由 `applies_to / scope / linked_assets` 进入 registry index 和行为边，`Flow Asset` 由 `involved_modules / architecture_fact_refs / validation_fact_refs` 推断，`Validator Asset` 由 `validates / bdd_refs` 推断
- `source: adapter`
  - 来自 `AdapterGraphProjection`，表示外部生产端、验证回流或人工 authoring 已通过 Adapter admission gate 写入的显式边
- `source: policy`
  - 来自 routing policy 或能力词表规则

`source` 取值固定为 `auto | registry | adapter | policy` 这一封闭集合。注意它与 `ContextBundle.intentResolution.source`（intent 来源）是不同字段，不要混用。

置信度规则：

- 外部生产端或人工确认的显式绑定高于 registry 类型字段推断
- registry 显式声明高于词表模糊匹配
- Behavior 明确 linked_assets 高于标签命中
- 路径命中高于自然语言标签命中
- 当前 build snapshot 中不存在的资产不可被 Router 看到；资产替换通过新的 build 产物表达，不通过运行时状态表达

#### 7.4.5 构建、校验与发布流程

`asset_graph` 的发布不是简单文件拷贝，而是一次带输入快照、校验门禁和产物冻结的 build 流程。authoring 环境中的工作图不得直接写入 `consumption/context_service`。

字段语义：

- `source_commit`
  - 本次 `asset_graph` 构建所依据的代码提交号
...
1. 固定输入快照
  ...
  - 读取 `Module Fact`
  - 读取 `Engineering Fact`
  - 读取 Adapter 输出的 `AdapterAssetProjection / AdapterGraphProjection`
  - 读取各资产目录的 `asset.registry.yaml`
  - 读取 `routing_policies / vocabularies`
2. 构建候选图
  - 生成 `behavior_id -> Behavior Asset` 索引
  - 生成 `Behavior -> Requirement / Specification / BDD / Validation / Validator / Architecture / Module / Engineering / Feature / Flow / Skill` 等关系边
  - 生成 `Requirement / Specification / BDD / Validation / Validator / Feature / Module -> Behavior` 的回到行为中心边
  - 为每条引用边写入 `source / status / status_source / reason / confidence`
3. 执行 graph validation
  - 校验所有节点是否存在于 registry index 或允许的 evidence / artifact ref 命名空间
  - 校验所有 `asset_id` 是否能解析到真实资产、registry 项或 evidence / artifact ref
  - 校验路径、registry 类型字段、边类型和引用边格式
4. 执行发布门禁
  - 存在 `error` 级 diagnostics 时阻断发布
  - required edges 缺失时阻断发布
  - 非 required edges 缺失时写入 diagnostics
5. 生成发布产物
  - 写入当前稳定 `asset_graph.yaml`
  - 写入 diagnostics
  - 写入 `graph_snapshot_id`
  - 写入 asset / policy / registry 快照引用
6. 发布到消费服务
  - 将通过门禁的 registry item、非 item-only 资产正文、`asset_graph`、registry index、Skill 摘要、validator 摘要和相关索引发布到 `consumption/context_service`
...
- `domains/<domain>/behavior_assets/`
  - 存放通过 build gate 的 `Behavior Asset`
  - Router 用它做 query、issue、BDD、spec 和 diff 的行为中心定位
- `domains/<domain>/modules/<module>/module_facts/`
  - 存放通过 build gate 的 `Module Fact`
  - Router 用它做 `changedPaths -> package_root -> module_id -> behavior` 辅助定位
  - Skill 不直接扫描该目录，只通过 Router 读取结果
- `asset_graph/asset_graph.yaml`
  - 存放已冻结的当前消费 Asset Graph
  - Router 用它读取 `has_requirement / specified_by / has_bdd / validated_by / executed_by / constrained_by / implemented_by / belongs_to_feature / participates_in_flow / uses_skill / relates_to_behavior` 等关系边
  - 文件内必须包含 `source_commit`、`graph_snapshot_id` 和节点级 metadata
- `indexes/asset_graph.diagnostics.yaml`
...
# 快照输入来源
inputs:
  # Behavior Asset 快照 ID
  behavior_assets_snapshot:
  # Module Fact 快照 ID
...
2. 读取当前快照中的 `asset_graph/asset_graph.yaml`
3. 根据 query、issue、BDD、spec、changedPaths 或显式 asset id 定位行为入口
4. 根据 `asset_graph.edges` 和 routing policy 选择候选资产
...
Router 正常加载只读取 build 后的当前稳定 `asset_graph`。authoring 环境中的工作图不直接进入代码助手消费链路。人工复核使用 diagnostics、trace 或指定资产读取接口，不引入独立的运行时资产状态模型。
...
Asset Graph 落地命令由 7.4.5 的发布流程定义，最小能力包括：
...
- `context:asset-graph:scan`
  - 从工程生成 authoring 侧 `Module Fact / Engineering Fact` 和基础依赖图
- `context:asset-graph:check`
  - 校验 `asset_graph`、registry 类型字段、路径和引用边，不写入 consumption
- `context:asset-graph:build`
  - 通过 build gate 后同步 registry item、非 item-only 资产正文、`asset_graph`、diagnostics 和 graph snapshot 到 `consumption/context_service`

### 7.6 边界约束

- 代码事实扫描不做最终业务判断
- `Module Fact` 不保存完整代码摘要
- `Engineering Fact` 不替代 `Architecture Fact / Validation Fact`
- 模块不作为默认导航根；它只通过 `implemented_by` 边参与行为中心导航
- authoring 中间态不直接面向项目研发暴露
...
type RoutingPolicy = {
  ...
  expansionRules: Array<{
    ...
    via: "has_requirement" | "specified_by" | "has_bdd" | "validated_by" | "executed_by" | "constrained_by" | "implemented_by" | "evidenced_by" | "belongs_to_feature" | "participates_in_flow" | "uses_skill" | "relates_to_behavior";
    // 最大扩展深度
    maxDepth: number;
  }>;
...
4. 定位候选行为
  ...
  - 使用 `entryAssetIds` 直接定位 Requirement / Specification / BDD / Validation / Validator / Feature / Module 等入口资产，再沿 `relates_to_behavior` 回到行为中心
  - 使用 `changedPaths -> package_root -> module_fact` 定位实现表面，再沿 `relates_to_behavior` 或 `implemented_by` 反向索引定位相关行为
  - 使用 `entry_points / config_touchpoints / asset_touchpoints` 补充实现表面定位
5. 读取 Asset Graph
...
规模变大后，不允许一次性把所有资产塞给相关 Skill。上下文应该按照当前需求采用渐进式选择，让 Router 先定位行为中心，再沿 `asset_graph` 的高价值边生成精确清单和正文读取建议。
...
flowchart TB
    ...
    Policy --> Entry["Resolve Entrypoint<br/>解析 query / issue / BDD / spec / diff 入口"]
    Entry --> Behavior["Resolve Behavior Assets<br/>定位行为中心"]
    Behavior --> Graph["Read asset_graph edges<br/>读取资产关系边"]
    ...
    NeedDetails -->|Requirement / Spec / BDD| IntentAssets["Select Requirement / Specification / BDD<br/>选择需求 规格 场景清单项"]
    NeedDetails -->|Validation / Validator| ValidationAssets["Select Validation / Validator<br/>选择验证与执行资产清单项"]
    NeedDetails -->|Architecture / Module / Engineering| ImplementationAssets["Select Constraints / Implementation Surface<br/>选择约束与实现表面清单项"]
    NeedDetails -->|Skill / Flow / Feature| SupportAssets["Select Skill / Flow / Feature<br/>选择辅助资产清单项"]
    NeedDetails -->|no| Budget["Budget Check<br/>预算检查"]
    IntentAssets --> Budget
    ValidationAssets --> Budget
    ImplementationAssets --> Budget
    SupportAssets --> Budget
...
- `impact_analysis`
  - 需要沿行为、规格、架构事实、流程和实现表面扩展影响面
- `integration_validation`
  - 需要同时读取接入基线、流程资产、验证事实和验证工具绑定
- `regression_precheck`
  - 需要查找 BDD、历史验证事实、QA checklist 和 Validator 状态摘要
- 行为归属不明确的 query
  - 需要先用 issue、BDD、spec、changed paths、entry points 和 touchpoints 缩小行为范围
- 高风险行为
...
- query 只需要读取一个已知 `Behavior Asset` 或 `Module Fact`
- query 是 registry 查询
  - 例如“列出 account domain 可用 Skill 或 validator”
...
1. 初始化入口资产
  - 从 `targetBehaviors` 读取显式行为
  - 从 `entryAssetIds` 读取显式 Requirement / Specification / BDD / Validation / Validator / Feature / Module 入口
  - 从 `changedPaths -> package_root -> module_fact` 反推实现表面
  - 从 `entry_points / config_touchpoints / asset_touchpoints` 补充实现表面
2. 定位行为中心
  - 显式行为直接进入候选
  - Requirement / Specification / BDD / Validation / Validator / Feature 入口沿 `relates_to_behavior` 回到行为中心
  - Module Fact 入口通过 `implemented_by` 的反向索引回到行为中心
  - 如果无法定位行为，返回 `partial / insufficient`，并在 diagnostics 中提示需要显式行为或入口资产
3. 按 query intent 决定第一批边类型
  - `smoke_test` 先取 `has_bdd / validated_by / executed_by / implemented_by`
  - `impact_analysis` 先取 `has_requirement / specified_by / constrained_by / participates_in_flow / implemented_by`
  - `integration_validation` 先取 `specified_by / has_bdd / validated_by / executed_by / constrained_by / participates_in_flow / implemented_by`
  - `regression_precheck` 先取 `has_bdd / validated_by / executed_by / participates_in_flow`
  - `sdk_upgrade_check` 先取 `implemented_by / constrained_by / has_bdd / validated_by / executed_by`
4. 读取 `Asset Registry Item`
  - 不打开资产本体，只读取摘要、适用条件、schema、路径和预算信息
  - 根据 intent、行为、平台、场景、模块、验证触点和能力标签判断候选价值
5. 执行 policy / snapshot 筛选
  - 只保留当前消费快照中存在的资产
  - 跳过不匹配 intent、行为、平台、场景、模块、能力或验证触点的资产
6. 生成资产清单项和正文读取建议
  - 只为通过筛选且满足 policy 的资产生成摘要、读取指针、hash 和 `recommendedFetchMode`
  - `belongs_to_feature` 对应的是 Feature Asset，用于产品或交付归属，不负责引入 Requirement / Specification
  - `specified_by` 对应的是 Specification Asset，Requirement 的覆盖状态和实现证据由 Specification 的 coverage 字段引入
  - BDD Scenario Asset 可以通过 registry item、`has_bdd`、`bdd_refs` 索引或显式请求进入；代码助手需要完整场景分析和 Given / When / Then 时再按清单授权读取正文
  - `executed_by` 对应的是 Validator Asset
7. 执行 Context Sufficiency Gate
  ...
  - 输出 `sufficient / partial / insufficient`
8. 不足时执行有界扩展
  - 沿 policy 允许的 `edge_type` 扩展邻接行为和资产
  - 对实现表面只在需要影响面补充时扩展 `declared_dependencies / dependent_packages`
  - 对流程只沿 `participates_in_flow` 扩展共同参与的行为
  - 扩展深度不得超过 policy 的 `maxDepth`
9. 执行预算裁剪
  - 超过 token 预算时，优先保留当前行为资产和显式入口资产
  - 丢弃低 confidence、低相关性资产
  - 同优先级内按 `confidence` 降序、再按 `asset_id` 字典序作确定性 tie-breaker
10. 输出 `ContextBundle`

所有资产必须带 `reason`；所有被丢弃资产必须进入 diagnostics。

### 11.4 多行为与多实现表面遍历

跨模块 query 不走“先合并模块再遍历”的模糊路径，而是走多行为遍历。模块只作为实现表面和影响面补充；最终合并单位是行为和资产，而不是模块。

触发条件：

- `targetBehaviors` 数量大于 1
- `entryAssetIds` 命中多个行为
- `changedPaths` 命中多个 `package_root`，并通过反向边关联到多个行为
- query 中出现多个明确业务域，例如账号、支付、存档、广告、AB Test
- `asset_graph` 的 `participates_in_flow` 显示多个行为共同参与同一关键流程

输入：

```ts
type MultiBehaviorTraversalInput = {
  // 原始上下文加载请求
  request: ContextLoadRequest;
  // 当前使用的路由策略
  policy: RoutingPolicy;
  // 遍历根行为列表
  rootBehaviors: string[];
  // 辅助实现表面模块列表
...
```ts
type MultiBehaviorTraversalOutput = {
  // 每个根行为的独立遍历结果
  perBehaviorResults: Array<{
    // 行为 ID
    behaviorId: string;
    // 该行为选中的资产 ID 列表
...
1. 建立 root behavior 集合
  - 从 `targetBehaviors`、entry asset、changed paths、flow edges 和 touchpoints 得到根行为
  - 根行为去重后保持稳定排序
2. 单行为并行遍历
  - 每个 root behavior 独立执行 edge 扩展、snapshot 过滤和 sufficiency 初判
  - 单行为结果只产生候选资产，不直接写入最终 `ContextBundle`
3. 识别共享资产
  - 多个行为共同引用的 Architecture Fact 进入共享候选
  - 多个行为共同引用的 Validation Fact 进入共享候选
  - 多个行为共同参与的 Flow Asset 进入共享候选
  - 多个行为共同指向的 Module Fact / Engineering Fact 进入实现表面共享候选
4. 分配预算
  - 先预留全局 required slots 预算
  - 再为每个 root behavior 分配基础预算
  - 高风险行为、当前 diff 命中行为、显式目标行为获得更高预算权重
  - 共享资产只计一次 token，不按行为重复计费
5. 合并与冲突诊断
  - 相同 asset id 合并 reasons 和 sourceBehaviors / sourceModules
  - 不同版本基线、不同验证工具绑定口径或不同 flow 解释进入 `conflictDiagnostics`
  - 冲突不由 Router 判定业务对错，只作为 Skill 和人工复核的输入
6. 交给 Context Fuser
  - `Context Fuser` 对多行为候选资产执行最终去重、排序、裁剪和输出

多行为预算分配公式：

```text
behavior_budget_weight =
  explicit_target * 0.35 +
  changed_path_hit * 0.25 +
  risk_level * 0.25 +
...
- `explicit_target`
  - 行为来自 `targetBehaviors` 或显式入口资产时为 `1`，否则为 `0`
- `changed_path_hit`
  - 行为通过 `changedPaths -> module_fact -> implemented_by` 反向索引命中时为 `1`，否则为 `0`
- `risk_level`
  - 来自 `BehaviorAssetItem`、`ArchitectureFactItem` 或关联 `ValidationFactItem` 中的受控字段
  - 取值必须来自统一词表：`low / medium / high / critical`
  - 映射为 `low=0.25`、`medium=0.5`、`high=0.75`、`critical=1`
  - 字段缺失时默认 `medium=0.5`，并写入 diagnostics
- `flow_centrality`
  - 来自 `asset_graph.participates_in_flow` 的参与度
  - 计算为 `behavior_flow_count / max_flow_count_in_root_behaviors`
  - 分母为 `0` 时结果为 `0`
...
- 先定位 Behavior
  - `Behavior Asset` 和 `asset_graph` 索引体积小，能快速完成行为级缩圈
- 延迟加载 Architecture Fact / Validation Fact
  - 只有 query intent 需要时才读取大体积架构事实和验证事实
- 实现表面按需加载
  - `Module Fact / Engineering Fact` 只在 changed paths、影响面或 SDK 升级相关时进入核心候选
- 先固定 snapshot
  - 当前消费快照之外的资产不进入排序和预算计算
  - authoring 中间态不进入 Router 候选集合
- 有界扩展
  - 图边扩展必须受 `maxDepth` 和 `budget_profiles` 控制
- 先返回够用上下文
...
- 已定位到明确行为和对应 `Architecture Fact / Validation Fact`
- 已找到足够的验证口径和校验工具
...
- LLM 不得自由决定每一跳扩展
- 每一跳都必须来自 `routing_policies` 或 `asset_graph`
- 每一跳都必须记录 `reason`
...
## 14. Context Consumption Service、OpenViking 与 RAG
...
2. 建立 `Behavior Asset` 与 `asset_graph`，补充 `Behavior -> Requirement / Specification / BDD / Validation / Validator / Architecture / Module / Engineering / Feature / Flow / Skill` 等资产关系边
