...
- `Feature Asset`
  - 功能资产，用于描述 domain/module 下可交付的功能或能力单元；它聚合需求意图、设计意图、验收口径和 PR 来源引用，是 issue / PR 与代码模块之间的稳定语义层
- `Requirement Asset`
  - 需求意图资产，用于从 GitHub Project / Issue 等来源沉淀原始需求、设计意图、关键讨论结论和验收标准；它归属于 `Feature Asset`，不是代码事实，也不直接从 commit 推断意图
- `Flow Asset`
...
- `module_asset_graph`
  - 以模块为主要入口的跨层资产图，把 `Module Fact`、`Engineering Fact`、`Architecture Fact`、`Validation Fact`、`Feature Asset`、`Flow Asset`、`Skill Asset` 和 `Validator Asset` 连接起来；`Requirement Asset` 通过 `Feature Asset` 间接进入候选集合；它表达“可能相关”，不表达“本次必须加载”
...
- `authoring/domains/<domain>/modules/<module>/` 维护模块归属资产，包括 `Module Fact`、模块级 `Architecture Fact`、`Validation Fact`、`Feature Asset`、`Requirement Asset`、`Skill Asset` 和 `Validator Asset`
- `authoring/domains/shared/` 维护当前项目内跨 domain / 跨核心模块复用的工程基线、架构事实和流程资产，仅保留 `engineering_facts / architecture_facts / flow_assets`
...
- Feature、Requirement、Validation、Validator 和 Skill
  - 默认归属到具体 `domains/<domain>/modules/<module>/`
  - 其中 `Skill Asset` 是 module 级任务方法，不建议放到 feature 级
  - GitHub Project / Issue 如果经过 ML / 人工筛选后具备长期价值，应注册为 `Requirement Asset`，放入对应 module 的 `requirement_assets/`，并通过 `Feature Asset.requirement_refs` 归属到 feature
  - PR 不作为独立资产类型，只作为 `Feature Asset.source_refs` 或 `Requirement Asset.source_refs` 的来源引用

不新增 `refs.shared`。`domains/shared/` 只是项目内共享资产的物理 domain；消费模型上仍是普通资产，通过 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.flows / refs.skills / refs.validators` 被模块引用。

### 6.1 Asset Registry

`Asset Registry Item` 是每个资产目录下的 `asset.registry.yaml`。它不是集中手写的大注册表。它为所有可被 Router 独立发现、过滤、排序、裁剪和加载的资产提供静态身份证，包括 `Module Fact`、`Engineering Fact`、`Architecture Fact`、`Validation Fact`、`Feature Asset`、`Requirement Asset`、`Flow Asset`、`Skill Asset` 和 `Validator Asset`。
...
- `FeatureAssetItem`
  - 这个功能属于哪个 domain、默认归属哪个 module、跨哪些 module，聚合了哪些需求来源和关联事实
- `RequirementAssetItem`
  - 这条需求资产来自哪个 GitHub Project / Issue，沉淀了哪些原始需求、设计意图、讨论结论和验收标准，并归属于哪些 Feature
- `FlowAssetItem`
  - 这个流程涉及哪些模块、入口和关键链路
- `SkillAssetItem`
...
type FeatureAssetItem = BaseAssetItem & {
  ...
  // 涉及能力
  capability_tags?: string[];
  // 关联 Requirement Asset；Issue / Project 来源先归属到 Feature
...
type RequirementAssetItem = BaseAssetItem & {
  ...
  // 来源 URL
  source_url: string;
  // 归属 Feature；Requirement 不直接绑定 module
  feature_refs: string[];
  // 涉及能力
  capability_tags?: string[];
  // 需求意图摘要
...
`routing_policies` 是共享策略。它定义不同 query intent 如何加载 `Module Fact / Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Requirement Asset / Flow Asset / Skill Asset / Validator Asset` 等资产，以及每类请求的扩展深度、槽位要求和预算边界。
...
`module_asset_graph` 是以模块为主要入口的资产图，不局限于 L0。`L0` 提供自动生成的模块事实、依赖事实和工程事实，是资产图的主要输入之一；资产图负责把模块入口连接到 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset / Skill Asset / Validator Asset` 等跨层资产，让 Router 能从一个 query、diff 或目标模块稳定走到可用候选资产。
...
- `refs.features`
  - 当前模块相关的 Feature Asset，例如该模块默认负责或参与的可交付功能；GitHub Project / Issue 沉淀的需求意图通过 Feature 的 `requirement_refs` 间接进入上下文

这组边的目标，是让 agent 能从模块入口稳定走到 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset / Skill Asset / Validator Asset` 等所有可用候选资产，而不是退回到模糊匹配。`module_asset_graph` 不负责最终上下文选择；它提供候选全集，`routing_policies`、build 后的 registry index 和 `Context Fuser` 再按 intent、平台、场景和预算修剪候选集合，以获得最大上下文信噪比。
...
build gate 必须按 `edge_ontology.yaml` 校验所有 `refs` 下的分组键、允许的目标资产类型集合和 `source` 取值；未在词表中声明的边分组或来源值一律阻断发布。Router 和文档可以使用 `refs.l0` 这类概念路径表达扩展方向，但实现读取时必须解析为 `refs["l0"]`。这是边可扩展性的治理边界：`Feature Asset` 是模块到需求意图的语义桥接层；GitHub Project / Issue 沉淀为 `Requirement Asset`，再通过 `Feature Asset.requirement_refs` 进入上下文。PR 作为 `Feature Asset.source_refs` 或 `Requirement Asset.source_refs` 的来源引用保留，不进入默认图边。其它 tools 引入和业务事件类边（如复核、检查节点）都应通过同一套词汇扩展流程接入，而不是绕过词表新增隐式边。
...
`Module Fact` 只保存自动生成的包级事实。`Engineering Fact` 保存可从工程配置恢复的版本、平台和兼容矩阵事实。`module_asset_graph` 保存人工维护或半自动维护的候选导航边，从而让 `L0` 保持轻量，同时让 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset / Skill Asset / Validator Asset` 等资产的绑定关系独立更新。
...
- 把 `module:<package_name>` 映射到对应的 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset / Skill Asset / Validator Asset` 等资产
...
5. 读取 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset` 的 registry 类型字段
6. 使用 `module_tags`、`primary_module_ref`、`module_refs`、`involved_modules`、`architecture_fact_refs`、`validation_fact_refs` 和 `validator_bindings` 将上层资产挂到候选模块
7. 读取各资产目录的 `asset.registry.yaml`，根据资产声明的 `asset_type`、`applies_to`、`module_tags`、`capabilities`、`validation_touchpoints` 生成 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.flows / refs.skills / refs.validators`
...
12. 输出 `module_asset_graph.yaml` 和 diagnostics
...
- `source: registry`
  - 来自 `asset.registry.yaml` 显式声明，或 registry 类型字段推断；`Engineering Fact` 可由 `source_paths` 推断，`Architecture Fact / Validation Fact` 由 `module_tags / validation_touchpoints / review_status` 摘要推断，`Feature Asset` 由 `primary_module_ref / module_refs / requirement_refs` 推断，`Flow Asset` 由 `involved_modules / architecture_fact_refs / validation_fact_refs` 推断，`Validator Asset` 由 `validates` 推断
- `source: manual`
  - 来自人工确认的显式绑定
- `source: policy`
...
1. 固定输入快照
  ...
  - 读取 `Module Fact`
  - 读取 `Engineering Fact / Architecture Fact / Validation Fact / Feature Asset / Flow Asset` registry 类型字段
  - 读取各资产目录的 `asset.registry.yaml`
  - 读取 `routing_policies / vocabularies`
2. 构建候选图
  - 生成 `module_id -> Module Fact` 索引
  - 生成模块依赖边
  - 生成 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.flows / refs.skills / refs.validators`
...
- `asset_graph/module_asset_graph.yaml`
  ...
  - Router 用它读取 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.flows / refs.skills / refs.validators`
...
type RoutingPolicy = {
  ...
  expansionRules: Array<{
    ...
    // 扩展使用的 refs 边
    via: "refs.l0" | "refs.l1" | "refs.l2" | "refs.features" | "refs.flows" | "refs.validators" | "refs.skills";
    // 最大扩展深度
    maxDepth: number;
  }>;
};
...
6. 按 policy 扩展资产
  ...
  - 根据 `maxDepth` 控制遍历深度
  - `Requirement Asset` 只能由 `Feature Asset.requirement_refs` 或显式请求引入，不作为 `module_asset_graph.refs.*` 的第一跳边
7. 执行候选过滤
...
6. 输出 `ContextBundle`
  - 资产顺序固定为 `module_fact -> engineering_fact -> architecture_fact -> validation_fact -> feature_asset -> requirement_asset -> flow_asset -> skill_asset -> validator_asset`
  - 同类型内按 priority 降序排列
...
- `required_context`
  - 需要的上下文类型，例如 `module_fact`、`architecture_fact`、`validation_fact`、`feature_asset`、`flow_asset`
- `context_loading`
...
3. 按 query intent 决定第一批资产类型
  ...
  - `impact_analysis` 先取 `Module Fact + refs.features + refs.l1`
  - `integration_validation` 先取 `refs.features + refs.l1 + refs.l2 + refs.flows + refs.validators`
...
6. 按需加载资产本体
  ...
  - `refs.features` 对应的是 Feature Asset，Requirement Asset 由 Feature 的 `requirement_refs` 间接引入
...
- `Feature Asset` 是否通过 `primary_module_ref / module_refs` 绑定模块，并通过 `requirement_refs` 聚合需求意图来源；Feature 可以跨模块，但必须有一个默认归属模块
- `Requirement Asset` 是否通过 `feature_refs` 归属到 Feature，不直接声明 `module_refs`
...
- required refs 是否存在
- shared assets 中被消费的资产是否已注册为普通资产，并通过 `refs.l0 / refs.l1 / refs.l2 / refs.features / refs.flows` 被引用
